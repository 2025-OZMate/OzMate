...
export default function Translation() {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        if (questions[currentQuestion].correct.some(answer => answer === userInput.trim())) {
            setIsCorrect(true);
        } else {
            setIsCorrect(false);
        }
    ...
    return (
        <div className={styles["all-container"]}>
            <div style={{ display: "flex", columnGap: "8px" }}>
                <BackButton />
                <ProgressBar currentQuestion={currentQuestion} questionsLength={questions.length} />
            </div>
            ...
            <div className={styles["question-container"]}>
                ...
                {isCorrect === null && (
                    <div className={styles["block-container"]}>
                        {questions[currentQuestion].block.map((word, index) => (
                            <p key={index} className={styles["word-block"]}>{word}</p>
                        ))}
                    </div>
                )}
                ...
                {isCorrect === false && (
                    <div className={styles["emoji-container"]}>
                        <img src='/images/notCorrect.png' className={styles.notCorrect}></img>
                    </div>
                )}
                ...
                <form onSubmit={handleSubmit} className={styles["form-container"]}>
                    ...
                    >
                        <input type="text" value={userInput} onChange={(e) => setUserInput(e.target.value)}
                            ...
                            style={{
                                border: isCorrect === null ? "1px solid #FFB600" : isCorrect ? "1px solid var(--2CE32C, #2CE32C)"
